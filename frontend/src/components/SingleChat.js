    ...
    const sendMessage = async(event)=>{
        if(event.key === "Enter" && newMessage){
            ...
            try {
                ...
                setNewMessage(""); // To empty the input field after pressing the Enter key
                const {data} = await axios.post("/api/message", {
                    content : newMessage,
                    chatId: selectedChat._id,
                }, config);
                socket.emit('new message', data);
                setMessages([...messages, data]); // To add the new send data into the previous array of messages
